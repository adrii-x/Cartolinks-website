  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-1 ">
      ...
      {isExpanded && (
        ...
        >
          <menu className="-mx-3.5 grid grid-flow-row grid-cols-1 gap-0.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tools.map((tool, index) => (
              <li key={tool.title} className={index >= 8 ? 'hidden lg:block xl:hidden' : index >= 9 ? 'hidden xl:block' : ''}>
                <a 
